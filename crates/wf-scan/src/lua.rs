...
const ATOM_BASIS: u32 = 0x7e5a_f8e9;
const ATOM_PRIME: u32 = 0x0100_0193;
const ATOM_ROTATION: u32 = 15;
...
pub fn atom(name: &str) -> u32 {
    let hash = name.bytes().fold(ATOM_BASIS, |hash, byte| {
        (hash ^ u32::from(byte)).wrapping_mul(ATOM_PRIME)
    });
    (!hash).rotate_right(ATOM_ROTATION)
}
...
fn key_is<R: MemoryReader + ?Sized>(reader: &R, key: &[u8], name: &str) -> bool {
    match slot_tag(key).map(|tag| tag & KEY_TAG_MASK) {
        Some(ATOM_TAG) => read_u32_le(key, 0) == Some(atom(name)),
        Some(tag) if tag == u32::from(STRING_TAG) => {
            read_u64_le(key, 0)
...
impl LuaTable {
    ...
    pub fn field<R: MemoryReader + ?Sized>(self, reader: &R, name: &str) -> Option<LuaValue> {
        ...
        let node = nodes
            ...
            .0
            .iter()
            .find(|node| key_is(reader, &node[NODE_KEY..], name))?;
    ...
    pub fn values<R: MemoryReader + ?Sized>(self, reader: &R) -> Vec<LuaValue> {
        self.nodes(reader)
            ...
            .0
            .iter()
            .filter(|node| slot_tag(&node[NODE_KEY..]).is_some_and(|tag| tag & KEY_TAG_MASK != 0))
...
impl LuaState {
    ...
    fn at<R: MemoryReader + ?Sized>(reader: &R, heap: &[&Region], thread: u64) -> Option<Self> {
        ...
        let (globals_field, globals) = words(&fields)
            ...
            .find(|(_, table)| table.field(reader, SHARED_TABLE).is_some())?;
...
pub(crate) mod tests {
    ...
    #[test]
    fn shared_table_atom() {
        assert_eq!(atom(SHARED_TABLE), 0x7a3b_1179);
        assert_eq!(atom(""), 0x0e2d_034a);
    }
